...
![OSI](https://github.com/Dashiiidzzze/knowledge_base/blob/master/Pasted%20image%2020250322213924.png)
![[Pasted image 20250322213924.png]]
### Семь уровней:
- **Прикладной уровень (Application Layer)** - этом уроне работают привычные для нас приложения — e-mail, браузеры по протоколу HTTP, FTP и остальное. Но это не уровень приложений, это уровень протоколов их взаимодействия. То есть приложение определяет правила (протокол) по которому скомпановать данные, чтобы другое приложение смогло их прочитать и выдать человеку.
    
- **Уровень представления (Presentation Layer)** - выполняет три основные функции:
	- _Шифрование и дешифрование_ по протоколу [[Протокол TLS]] (Transport Layer Security). Шифрует мои данные и отвечает за расшифровку запросов, поступающих с других серверов, для использования на прикладном уровне.
...
- **Транспортный уровень (Transport Layer)** - обычно определяется в зависимости от используемого протокола. Данные на этом уровне называются сегментами. "Доставка от приложения к приложению".
	...
	Функции:
	- Сегментация и сборка - большой блок данных от прикладного уровня (например, весь HTML-код веб-страницы) и разбивает его на более мелкие части, называемые **сегментами** (в TCP) или **датаграммами** (в UDP).
	- Номера портов - использует номера портов (от 1 до 65535), чтобы идентифицировать конкретное приложение или службу
	- Мультиплексирование - ПК "распределяет" входящие данные по правильным приложениям, ориентируясь на порты.
	- Гарантия доставки и управление потоком (для TCP)
	- Установление соединения (для TCP)
    
- **Сетевой уровень (Network Layer)** - отвечает за передачу пакетов из сети в сеть, то есть за маршрутизацию. IP берет сегменты из транспортного протокола и добавляет метаданные. водит **логические, независимые от оборудования адреса** — **IP-адреса**. здесь данные называются пакетами.
...
- **Физический уровень (Physical Layer)** - преобразовает кадры в байты (8 бит) и отправить их по какому-либо способу транспортировки (электричество, волны, свет и так далее).
	- **В медном кабеле:** Биты кодируются изменениями **электрического напряжения**.
		- Например: `+5В` = логическая `1`, `-5В` = логическая `0`.
	- **В оптоволокне:** Биты кодируются **световыми импульсами**.
		- Например: `Свет есть` = `1`, `Света нет` = `0`.
	- **В Wi-Fi (радиоэфир):** Биты кодируются изменениями **радиоволн** (модуляцией).
		- Например, изменение частоты (FSK) или фазы (PSK).
