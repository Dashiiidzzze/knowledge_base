**Dynamic Host Configuration Protocol** (протокол динамической настройки узла) – сетевой протокол прикладного уровня, обеспечивающий предоставление IP-адресов и сведений о сети для хостов, которые хотят подключиться к сети. После получения данных параметров хост становится полноправным узлом сети и может обмениваться пакетами в сети.
...
ip dhcp pool MY_POOL
 network 10.10.30.0 255.255.255.0      # Указываем сеть для раздачи адресов
 default-router 10.10.30.1             # Шлюз по умолчанию (адрес интерфейса)
 lease 7                               # Срок аренды адреса (в днях)
 exit
```
**Исключаем зарезервированные адреса (если нужно):**
`ip dhcp excluded-address 10.10.30.1 10.10.30.10   # Не выдавать эти адреса`
