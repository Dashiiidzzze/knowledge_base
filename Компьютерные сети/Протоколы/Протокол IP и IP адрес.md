...
# IP адрес
**IP-адрес** (от англ. Internet Protocol) — уникальный числовой идентификатор устройства в компьютерной сети работающей по протоколу IP. (протокол сетевого уровня).
...
![IP tabl](https://github.com/Dashiiidzzze/knowledge_base/blob/master/Pasted%20image%2020250322170928.png)
...
здесь **бинарная нотация** - это двоичное представление IP адреса.
![IP vichisl](https://github.com/Dashiiidzzze/knowledge_base/blob/master/Pasted%20image%2020250326184013.png)
![[Pasted image 20250326184013.png]]

В зависимости от результата сравнения, компьютер действует по-разному
##### Сценарий А: Адресат в моей сети
- **Решение:** Отправить пакет напрямую.
- **Действие:** Компьютер использует протокол **ARP (Address Resolution Protocol)**.
    1. Он "кричит" в локальную сеть: "Эй, у кого IP-адрес `192.168.1.15`? Скажи мне свой MAC-адрес!"
    2. Устройство `192.168.1.15` откликается: "Это я, мой MAC-адрес `BB:BB:BB:BB:BB:BB`".
    3. Ваш компьютер отправляет пакет напрямую на этот MAC-адрес через коммутатор.

> **Итог:** Шлюз (маршрутизатор) в этом процессе **не участвует**.
##### Сценарий Б: Адресат в другой сети
- **Решение:** Отправить пакет шлюзу по умолчанию.
- **Действие:** Компьютер понимает, что сам не может "достучаться" до чужой сети.
    1. Он использует ARP, чтобы найти **не IP-адрес цели, а MAC-адрес шлюза по умолчанию** (например, `192.168.1.1`).
    2. Он отправляет пакет на MAC-адрес шлюза, как бы говоря: "Вот, маршрутизатор, разберись с этим, ты знаешь пути наружу".
    3. Дальше шлюз берет на себя заботу о передаче пакета дальше по цепочке к цели.
...
![IP header](https://github.com/Dashiiidzzze/knowledge_base/blob/master/Pasted%20image%2020250322174632.png)
