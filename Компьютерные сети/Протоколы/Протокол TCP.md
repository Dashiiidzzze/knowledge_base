...
1) Клиент инициирует соединение, то есть отправляет сегмент данных с порядковым номером и флагом SYN.
2) Сервер отвечает сегментом с двумя флагами — SYN и ACK. Последний подтверждает, что отправленные инициатором данные были получены. При этом сервер увеличивает порядковый номер принятого сегмента на 1 и записывает это значение в поле номера подтверждения. И последний этап второй ступени рукопожатия — установка произвольного номера для ответного пакета.
3) После принятия клиентом фрагмента с SYN+ACK соединение считается установленным. Далее происходит отправка ответного сегмента с ACK и обновленными номерами. Этот пакет не несет смысловой нагрузки — он является проверочным.
